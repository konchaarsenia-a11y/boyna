...
|-------|-----------------|-------|
| **Чистовик** (active / Бойня) | — | Склад, Доступы, Клиенты, Прием заказов, Нарезка, Доставки, Брони, CRM (Контакты/ПП/АФК/БП/месяцы) после переноса |
...
### CRM-месяцы (не переупаковывать)

Формат уже рабочий:
- имя листа: `Июль` / `Август` … (опционально `Июль 2026`);
- строка 1: числа дней `1`, `3`, `6`… (или даты);
- ячейка: ник, адрес, сегмент ПП/АФК/БП/Р, телефон.

Год берётся из даты доставки при синке в брони. Один общий шаблон не нужен — риск потерять клиентов.

После деплоя один раз: `setupOpsEcosystem()` — зальёт всех в лист **Клиенты** (upsert, никого не удаляет). Проверка: `GET …&action=crmInventory&callback=cb`.
